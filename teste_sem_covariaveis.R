...
# Calcular os valores correspondentes de a
a_vals <- rho_vals * (1 - p0) / p0

# Criar um data frame
df <- data.frame(rho = rho_vals, a = a_vals)

# Calculando p0

df <- df |>
  dplyr::mutate(p0 = rho / (rho + a))

# Visualizar os pares (a, rho)
head(df)

# (Opcional) plotar as combinações
plot(df$rho, df$a,
  type = "l", col = "blue", lwd = 2,
  xlab = "rho", ylab = "a", main = paste("Combinações que satisfazem p0 =", p0)
)
